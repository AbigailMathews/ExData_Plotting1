...
# This script creates a histogram of Global Active Power from a subset of the 
# household power consumption dataset. Because the source data is contained in a zipped
# archive, this program assumes that the file has been downloaded ahead of time and
...
loadpower <- read.table("household_power_consumption.txt", na.strings='?', sep=";", header=TRUE, colClasses=c("character","character",rep("numeric",7)))

# Fix up header
colnames(loadpower) <-names(read.table('household_power_consumption.txt', header=TRUE,sep=";",nrows=1))
...
# Create the histogram plot 1 in a png file, plot1.png
png(file = "plot1.png")
